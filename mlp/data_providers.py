...
class MNISTDataProvider(DataProvider):
    ...
    def to_one_of_k(self, int_targets):
        ...
        for t in int_targets:
            ...
            temp[t]=1
            v_targets = np.vstack((v_targets, temp)) if v_targets.size > 0 else temp
        return v_targets
...
class MetOfficeDataProvider(DataProvider):
    ...
    def __init__(self, window_size, batch_size=10, max_num_batches=-1,
        ...
        date = loaded[:, 0:2].astype('int16')
        perc_values = loaded[:, 2:] 
        perc_values = perc_values[perc_values > -99.8];
        ...
        normalized_values = (perc_values-mean)/std
        
        # convert from flat sequence to windowed data
        if normalized_values.shape[0] % window_size != 0:
            normalized_values =normalized_values[: -(normalized_values.shape[0] % window_size)]
            
        windowed_data = normalized_values.reshape((-1, window_size))
